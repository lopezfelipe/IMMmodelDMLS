...
max_T = 2560.0; % Suggested max. temp. in the grid (ºC)
...
alpha_0 = ThermalDiffusivity(T_max); % Thermal diffusivity evaluated at T_max
y_nom = zeros(n,1); % Initial guesses of half widths
...
width_vector = 2.0e+6*b; % Vector of maximum widths
% Plot
if plot_flag
    figure (1)
...
d_T = dT; % Temp. grid size
width.laser = 2.0e+6*y_array(end,m); % Array of melt pool widths under laser
width.max = width_vector(end); % Array of max. melt pool widths
end
