...
% (cmontogom@andrew.cmu.edu). Comparisons are shown for different speeds at
% 195 W.
...
% Experimental data
e = 10.4*ones(size(v)); % 95% confidence interval for measurement error
...
figure(1)
errorbar(1.0e+3*v,widths(4,:),e,'bd','LineWidth',2.0); hold on;
plot(1.0e+3*v(2:end-1),pred_widths(3,:),'b');
plot(1.0e+3*v(2:end-1),pred_widths(3,:)-(pred_widths(3,4)-widths(3,4)+UQ_error)*ones(1,4),'b');
plot(1.0e+3*v(2:end-1),pred_widths(3,:)-(pred_widths(3,4)-widths(3,4)-UQ_error)*ones(1,4),'b');
xlabel('Scan speed (mm/s)'); ylabel('Width (\mum)');
xlim([0 1400]); ylim([0 300]); grid on;